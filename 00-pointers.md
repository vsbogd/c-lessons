...
гигабайт, все же это гораздо меньше, чем 18 квинтиллионов байт.
...
Важно понимать, что даже если две переменных, объявленных в разных блоках
программы, имеют одно и то же имя, они все равно будут иметь разные адреса,
...
новое место. Или, например, функция меняет только маленький кусочек данных. В
этом случае данные придется скопировать два раза: один раз, чтобы передать их в
функцию, а второй раз, чтобы вернуть результат.
...
компилятор знает сколько данных лежит по указателю? На это указывает тип
указателя, который состоит из типа данных, на которые ссылается указатель и
символа `*`, который является признаком типа указатель. Например, объявление
`int *p;` говорит компилятору о том, что `p` - это переменная содержащая адрес
в памяти (`*`) целого числа (`int`). Кстати, пробел можно ставить в любом
месте, и записи: `int* p;`, `int *p;` и `int*p;` - будут означать одно и то же.
...
Например, часть адресов зарезервирована для обращения к различным устройствам
компьютера. Запись и чтение в такие адреса не помещает значения в оперативную
память, а передает их для обработки устройству расширения. Такая нестрогость C
это одна из причин его популярности в низкоуровневой разработке. C позволяет
манипулировать памятью напрямую, что более строгие языки, вроде Java, делать не
позволяют.
